...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template match="/">
        <html>
            ...
            <body>
                ...
                <!--<xsl:apply-templates
                select="/energie-data/energie-plant">-->
                <!--</xsl:apply-templates>-->
    ...
    <xsl:template match="price">
        ...
        <xsl:if test="position() mod 10 = 0">
            <path
                d="M 0{position() div 2 * $x-per-step + 50 - $x-per-step * 6} 0{./preceding-sibling::price[5] * -20 + $baseline} L 0{position() div 2 * $x-per-step + 50} 0{. * -20 + $baseline}"
    ...
    <xsl:template match="plant">
        ...
        <svg width="1100" height="{$baseline + 30}" xmlns="http://www.w3.org/2000/svg">
            <rect x="0" y="0" width="1100" height="{$baseline + 30}" style="fill: #303030" />
            <path d="M 0 {$baseline - 1  } L 1100 {$baseline - 1  }"
                style="stroke: #505050; stroke-width: 2" />
            <path d="M 0 {$baseline - 100} L 1100 {$baseline - 100}"
                style="stroke: #505050; stroke-width: 2" />
            <path d="M 0 {$baseline - 200} L 1100 {$baseline - 200}"
                style="stroke: #505050; stroke-width: 2" />
            <path d="M 0 {$baseline - 300} L 1100 {$baseline - 300}"
                style="stroke: #505050; stroke-width: 2" />
